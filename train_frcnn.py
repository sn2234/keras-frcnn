...
(options, args) = parser.parse_args([
    '-p', 'D:\\Prog\\Projects\\MachineLearning\\Misc\\Keras\\keras-frcnn\\VOCdevkit\\',
    '--num_epochs', '100',
    '--num_rois','15'])
...
#        try:
            ...
            print("Training model_rpn completed with final loss: {0}".format(loss_rpn))

            P_rpn = model_rpn.predict_on_batch(X)
            print('Predict PRN completed')

            R = roi_helpers.rpn_to_roi(P_rpn[0], P_rpn[1], C, K.image_dim_ordering(), use_regr=True, overlap_thresh=0.7, max_boxes=300)
            print('rpn_to_roi completed')

            # note: calc_iou converts from (x1,y1,x2,y2) to (x,y,w,h) format
            X2, Y1, Y2 = roi_helpers.calc_iou(R, img_data, C, class_mapping)
            print('calc_iou completed')
            ...
            print('Training model_classifier')
            loss_class = model_classifier.train_on_batch([X, X2[:, sel_samples, :]], [Y1[:, sel_samples, :], Y2[:, sel_samples, :]])
            print('Training model_classifier completed with loss: {0}'.format(loss_class))
